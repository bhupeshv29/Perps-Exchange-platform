...
export async function loadSnapshot(): Promise<void> {
  try {
    ...
    Object.assign(orders, snapshot.orders);

    Object.assign(orderbooks, snapshot.orderbooks);

    Object.assign(markPrices, snapshot.markPrices);

    console.log("snapshot loaded", new Date(snapshot.createdAt));
  } catch (error) {
    console.log("no snapshot found or failed to load snapshot", error);
  }
}
